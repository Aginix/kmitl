...
export class Revenue_budget extends Component {
    ...
    updateNote = async (note, data) => {
        if (!data.plan_line) {
            await this.orm.create("budget.appropriation.line", [
                {
                    appropriation_id: this.state.budget_plan.budget_plan_data.id,
    ...
    onBlurSavePlan = async (pos) => {
        const data = await this.orm.write("budget.appropriation.line", [pos.plan_line.id], {
            fund_analytic_id: this.state.fund,
    ...
    async fetchData() {
        ...
        const budget_template_line_can_edit = get_structure_template_line["lines"].map(
            (item) => ({
                ...item,
