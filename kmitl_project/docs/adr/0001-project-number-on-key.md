...
- The sequence uses `use_date_range` so each fiscal year gets its **own** `no_gap` counter that resets to `0001` — numbers never run mixed across years. The per-year bucket is the calendar-year date range containing `account_fiscal_year_id.date_to` (each fiscal year ends in a distinct calendar year, so one range per FY); the range is auto-created on first use, and a `16.0.2.0.2` post-migration seeds each existing year past its highest issued number to avoid colliding with codes minted under the former single global counter. (`procurement_plan` still uses a single global counter.)
