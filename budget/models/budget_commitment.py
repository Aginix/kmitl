...
class BudgetCommitment(models.Model):
    ...
    def _create_reserve_line_from_header(self):
        ...
        same data a second time through a dialog. Called by
        :meth:`action_reserve` only when no reserve line exists yet, which keeps
        every programmatic creator (project/plan/mixin hosts pass ``line_ids``
        themselves) untouched. Cross-charge (multi-code) reservations are the
        extension module's concern and override this hook.
        """
        self.ensure_one()
        self.write(
            {
    ...
    # The reservation picker (amounts mode) on the commitment itself lives in
    # the budget_cross_charge extension: the core journey is form-first — the
    # header IS the single reserve line. The picker JS component stays here as
    # shared infrastructure (budget.commitment.mixin uses its select-only mode).
