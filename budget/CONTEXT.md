...
In bookkeeping terms the system keeps **separate books**: `budget.move` is the **main ledger** (บัญชีแยกประเภทงบ — actual money: appropriation / transfer / disbursement) and `budget.commitment` is a distinct **encumbrance register** (สมุดทะเบียนคุมเงินจอง/ผูกพัน) that controls reservation and obligation. Two books by design — the ledger records money that has *moved*, the register controls money that is only *earmarked*. This is why reserve/obligate live only on the commitment and never as a `budget.move` type.
...
A `budget.commitment` created **directly** as a first-class document — reserved up front against a budget pool with no originating source document — and later **picked up** by consuming documents that obligate/consume against it. Generalises the shared-commitment pattern: previously only a `procurement.plan` or `kmitl.project` could own a pre-reserved commitment, now a commitment may also stand alone. The reservation slip and the consuming document may belong to **different Operating Units** (central reserves, a unit spends). A slip with **no source** is spendable through a คำขออนุมัติ (`approval.request`) but **not** through a ใบขอซื้อ (พ.1), which may only draw a slip backed by a โครงการ or a แผนจัดซื้อจัดจ้าง ([ADR-0015](./docs/adr/0015-pr-draws-only-source-backed-reservations.md)).
_Avoid_: allocation, earmark (use "reserve"); BC on its own is the *reservation slip*, not the spend.

**Draw down (หยิบใบจองไปใช้)**:
The act of a consuming document (PR / disbursement) **linking to an existing reservation** (`budget_commitment_id`) instead of reserving its own — after which its spend obligates/consumes against that shared commitment. A drawing document never re-reserves and inherits the reservation's dimensions + fiscal year locked. One reservation may be drawn by many documents up to its cap (plan keeps its one-active-PR rule; project and standalone allow many). **Which slips a document may draw is a property of the document, not of the slip**: a ใบขอซื้อ (พ.1) draws only source-backed slips, a คำขออนุมัติ draws any it can see ([ADR-0015](./docs/adr/0015-pr-draws-only-source-backed-reservations.md)).
_Avoid_: consume (that is only the final leg); draw-down is the linking, obligate/consume are what follow.

**แหล่งงบประมาณของ พ.1 (Budget Source of a Purchase Request)**:
The single question a ใบขอซื้อ answers about where its money comes from, with a **closed set of three** answers: **ผังงบประมาณ** (reserve anew), **โครงการ/กิจกรรม**, and **แผนจัดซื้อจัดจ้าง** — the latter two being a draw-down of that source's own reservation. A source-less ใบจองงบประมาณ is not one of the answers. Each answer is owned by the module that owns the source, so the set grows only by adding a source, never by configuration.
_Avoid_: วิธีเลือกงบประมาณ (the earlier label — this axis is the *source* of the money, not the *method* of choosing it), mode/โหมด.
...
**Budget Ledger (สมุดรายการเคลื่อนไหวงบประมาณ)**:
A chronological, line-level report that opens a budget **book** on screen so activity reads at a glance instead of record by record. It reads the two books *separately* (see Disbursement side): the **main ledger** (`budget.move.line` — จัดสรร / โอน / เบิกจ่าย, shown with a running งบคงเหลือ) and, as its own book, the **encumbrance register** (`budget.commitment.line` — จอง / ผูกพัน). A read-only surface: holds no state, stores nothing, re-presents rows the books already own. Filtered by fiscal year and the six dimensions. The books are **not** summed into one balance — each carries its own (main = งบคงเหลือ; register = ยอดจอง/ผูกพันคงเหลือ). The main ledger's running คงเหลือ is **accounting remaining** ((a) − เบิกจ่าย, *before* encumbrance) — deliberately distinct from Remaining (f), which also nets เงินจอง held in the register; the (f) figure is surfaced from the monitoring dashboard summary, never re-derived in the running column (which the single book cannot do). Merging the books into one interleaved stream is a possible future read-model, never a change to `budget.move` (ADR-0010).
_Avoid_: movement (too vague on its own), Budget Move (that is the main-ledger *document* — one book, not the whole report)
