...
> **Reserve trigger revised** by [kmitl_project ADR-0002](../../../kmitl_project/docs/adr/0002-approval-gated-lifecycle-esaraban.md): reservation moved from the old `draft→new` Confirm to the `to_verify→to_send` "จองงบประมาณ" step of the project's e-Saraban approval. The floating-budget model in this ADR is otherwise unchanged.
