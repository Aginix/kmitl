...
- **Form preview:** once the หนังสือ is sent, the official PDF previews **inline in the form** — a `sarabun_pdf_inline` view widget rendering an `<iframe>`, placed **before the เนื้อหา section**. A **draft** has no inline preview and instead opens the PDF in a **preview dialog** via a "ดูตัวอย่างเอกสาร" button (`sarabun_pdf_preview` client action, `target=new`). Two earlier attempts were dropped — an in-form overlay that inherited `web.FormView` (fragile), then a full-page client action (awkward). Requires the PDF controller to serve **`inline`** (today it forces `attachment`).
