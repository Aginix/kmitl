...
- **Form preview:** the backend form shows the PDF **preview by default once `state != draft`** (embedded `<iframe>` served by `/sarabun/document/<id>/pdf`), with a toggle to the editable form and back; a draft defaults to the form but keeps a "view preview" toggle. Requires the PDF controller to serve **`inline`** (today it forces `attachment`).
