...
> **Update (2026-07-23 · [ADR-0010](./0010-register-number-at-completion-not-at-send.md)):** the **numbering timing** below is superseded. ลงทะเบียน no longer fires at send; the official number runs only at **completion** (final ลงนาม/อนุมัติ). A หนังสือ is therefore **unnumbered while circulating**, and reject/cancel — both pre-completion — find **no number to void** (the *voided-number/permanent-gap* mechanism becomes a no-op on the normal path; abandoned documents consume no counter).
