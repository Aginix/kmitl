...
A journal entry with more than one line on either side of the selected account (e.g. a multi-line bill, a payment split by fund/department, withholding tax). The Thai ledger convention is a **balance-form** report — one row per line of the account being viewed, using **that line's own** debit/credit, never a value computed from the opposite side. The *Account* column is a label only: the single counterpart account's name when there is exactly one, `_("Various accounts")` (บัญชีต่าง ๆ) when there are several, or the selected account's own name when there is none. This keeps every account's Debit/Credit/carried-forward tied to its own `account.move.line` rows, matching the Trial Balance — the report never re-derives an account's amount from a different account's lines.
_Avoid_: exploding a line into one row per counterpart (double-counts the opposite side's total on every account it touches)
