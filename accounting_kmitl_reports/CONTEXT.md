...
The detailed sibling of the Trial Balance, laid out like the KMITL web ledger. The menu opens a **wizard** (which accounts to report + the date range; empty accounts = all) whose button launches the report. The report is an **OWL screen**: per selected account, an opening line (ยอดยกมา), then one **row per journal item** of that account in date order, then a carried-forward line (ยอดยกไป). Each row's columns are **Date | Journal | Issue | Account | Remark | Partner | Debit | Credit | Balance** (running) — where *Journal* is the source book's code, *Issue* is the journal entry number (`account.move.name`), *Account* is the entry's counterpart account (see *Compound entry*) and *Remark* is the line's own description (`account.move.line.name`, falling back to `ref` then the entry narration). *Journal*, *Remark* and *Partner* are there because the 2544 notification requires the ledger to cite the type of book it was posted from, to describe the increase/decrease or the source of the revenue/expense, and to name the debtor/creditor. Dates print in the **Thai Buddhist era** (`2 ต.ค. 2568`, via `thai_date_utils`) as `date_display`; the raw ISO `date` is kept and is what the CSV exports, which is meant to be pivoted. The PDF header repeats the **active filters** under the period (`_kmitl_filter_summary`) so a printed extract can be reproduced. Clicking a row **expands** it (the same panel style as the workflow module's Approval List): an info block with the date, maker + date, partner and the KMITL budget dimensions (fund / department / activity / source, resolved from the line's `analytic_distribution`), the narration, and a bordered table of the **full journal entry's Dr/Cr lines** (Account, Label, Debit, Credit) lazily fetched from `account.move.line`; the panel also links to the journal entry (`account.move`). The period's lines come from the OCA `general_ledger` engine (flat — `grouped_by="none"`, no centralization, single company, company currency), but the **opening balance is ours**: `_kmitl_opening_balances` on `dimension.filter.mixin`, the same helper the Trial Balance uses, so the two reports cannot report a different ยอดยกมา for the same account (see *Opening balance*). The wizard params travel to the client action in `action.params`. Shares its compute with the QWeb PDF and XLSX exports (flat, no expand).
_Avoid_: GL, journal report (a journal report lists by journal, not by account)

**Compound entry (รายการผสม)**:
A journal entry with more than one line on either side of the selected account (e.g. a multi-line bill, a payment split by fund/department, withholding tax). The Thai ledger convention is a **balance-form** report, so an account's **amount** is always its own `account.move.line` figure — never a value computed from the opposite side. The *Account* column names the **counterpart** (`_kmitl_split_by_counterpart`): one counterpart is named outright; several **whose amounts add up to the line** become **one row each** carrying that counterpart's own figure (`Dr เจ้าหนี้ 60,000 / Cr ธนาคาร 59,439.25 / Cr ภาษีหัก ณ ที่จ่าย 560.75` → two rows); several that **do not add up** stay **one row for the whole amount, labelled with every account it faces**, because the entry pairs none of them to this line — two unrelated settlements booked in one move, an account posted on both sides, or several lines of this account against several others. Splitting is presentation only: the parts always add back to the line's own debit/credit, so every account's Debit/Credit/carried-forward stays tied to its own move lines and matches the Trial Balance. Expand a row to see the entry as posted.
_Avoid_: letting a counterpart's amount become the row's amount (double-counts the opposite side's total on every account it touches); apportioning a line across counterparts the entry never paired it with (invents amounts — `Dr WHT 500 / Cr CA 500 / Dr CA 500 / Cr SA 500` must not print 250 + 250); a catch-all counterpart label

**Opening balance (ยอดยกมา)**:
An account's balance as of the day before `date_from`, computed by `_kmitl_opening_balances` on `dimension.filter.mixin` and shared by the Trial Balance and the General Ledger. Read in two passes because the two account families restart differently: balance-sheet accounts (`include_initial_balance`) accumulate **every** entry before `date_from`; profit & loss accounts accumulate only from the **fiscal-year start**, so รายได้/ค่าใช้จ่าย open at 0.00 on 1 ต.ค. The fiscal-year start comes from the declared `account.fiscal.year` covering `date_from` (`_kmitl_fy_start_date`), falling back to the company's year-end setting. Every filter applied to the period — company, journals, partners, posted-only, accounts, dimensions — is applied identically here, or the block would not foot.
_Avoid_: computing it from the OCA engine (`_get_initial_balance_data`); upstream has already silently changed it for P&L accounts on the `grouped_by="none"` path, and it would drift from the Trial Balance
