...
A journal entry with more than one line on either side of the selected account (e.g. a multi-line bill, a payment split by fund/department, withholding tax). The Thai ledger convention is a **balance-form** report, so an account's **amount** is always its own `account.move.line` figure — never a value computed from the opposite side. The *Account* column names the **counterpart**, and a line facing several counterparts is shown as **one row per counterpart account** (`_kmitl_split_by_counterpart`) rather than under a catch-all label, which tells the reader nothing. The split is **presentation only**: the parts always add back to the line's own debit/credit, so every account's Debit/Credit/carried-forward stays tied to its own move lines and matches the Trial Balance. Counterpart figures are the weights, so in the ordinary case — one line of this account against N others, e.g. `Dr เจ้าหนี้ 60,000 / Cr ธนาคาร 59,439.25 / Cr ภาษีหัก ณ ที่จ่าย 560.75` — each row carries the counterpart's exact figure. When **both** sides carry several accounts no true pairing exists, so the weights merely apportion and the last row absorbs the rounding residual; the expand panel always shows the entry as posted.
_Avoid_: letting a counterpart's amount become the row's amount (double-counts the opposite side's total on every account it touches); a catch-all counterpart label
