# FIXME(rebase 2026-09): origin/16.0 deleted this whole models/ package
# (module rewritten onto base.automation + a new purchase_request_budget_todo
# module — see purchase_request_todo/data/base_automation.xml and
# purchase_request_budget_todo/), and its own reserve-budget automation is
# still keyed on the pre-refactor to_verify/to_submit state strings this
# branch retires. Restored here (kept, not deleted) per this branch's design
# — needs reconciling with the upstream rewrite before merge: either port
# this logic onto base.automation too, or make purchase_request_budget_todo's
# automation state filters match to_verify_budget/to_approve/sent.
...
class PurchaseRequest(models.Model):
    ...
    # ---------------------------------------------------------------------
    # Reserve Budget (to_verify → to_verify_budget)
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # Awaiting endorsement letter (to_verify_budget → to_approve)
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # Awaiting authorized signer (to_approve → sent, Sarabun routing)
    # ---------------------------------------------------------------------
    ...
    # ---------------------------------------------------------------------
    # Endorsement approved (sent → in_approval / in_egp)
    # ---------------------------------------------------------------------
    ...
    def button_to_approve(self):
        ...
        # Two landing states share this one override — ธุรการ ส่งต่อ at
        # to_verify diverts to to_verify_budget (purchase_request_budget), and
        # the budget officer's own call to button_to_approve (from
        # action_reserve_budget) lands at to_approve. Filter by where records
        # actually landed rather than assuming either always happens.
        self.filtered(
            lambda r: r.state == "to_verify_budget"
    ...
    def _on_sarabun_circulating(self, document):
        ...
        landed = self.filtered(lambda r: r.state == "sent")
