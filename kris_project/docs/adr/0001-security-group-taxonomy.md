...
The staff/viewer groups form a single implication chain — `manager` ⊃ `officer` ⊃ `viewer` ⊃ `base.group_user` — so Settings → Users renders the category as one ordered role dropdown. Odoo only renders a selection dropdown when implication yields a *total order* over a category's groups (`res.users.get_groups_by_application` → `linearize`); independent siblings tie and fall back to ambiguous checkboxes. `ou_viewer` stays off this chain (different audience, OU-scoped).
...
- **Independent sibling groups (`viewer` and `officer` both implying only `base.group_user`)** — rejected: it kept `officer` free of the viewer record rule, but the category then had no total implication order, so Settings → Users showed confusing checkboxes instead of a role dropdown. The linear chain reads clearly when assigning users.
...
- Because `officer` implies `viewer`, every officer also carries the viewer record rule (own projects). The per-model `*_full_rule [(1,'=',1)]` (groups officer+manager) is therefore load-bearing — it OR-restores full visibility for officers/managers and must not be removed.
