...
REPORT_MODEL = "finance_kmitl_reports.receivable.due.report"
CARRIER_MODEL = "finance_kmitl_reports.receivable.due.report.wizard"
...
class ReceivableDueReport(models.AbstractModel):
    """รายงานลูกหนี้ถึงกำหนดชำระ — the mirror of the payables due report, read
    the same way: what is owed to KMITL, and by when.

    One row per posted ``out_invoice`` that is not yet settled, ordered by its
    due date. Forward-looking, which is what separates it from **Aged
    Receivable** in ``accounting_kmitl_reports`` — see ``CONTEXT.md``.
    """

    _name = REPORT_MODEL
    _inherit = "finance_kmitl_reports.report.base"
    _description = "KMITL Receivable Due Report Data Provider"
    ...
    def _kmitl_domain(self, options):
        """Everything still owed to KMITL and falling due on or before
        ``date_to``.

        ``payment_state`` is borrowed from the accounting dashboard's
        ``_UNPAID_STATES`` rather than restated, so the card the accounting
        office reads every morning and this report cannot disagree about
        which invoices are still owed.
        """
    ...
    def _kmitl_row(self, move, today):
        ...
        row = {
            ...
            # Blank rather than 0 for an invoice that is not late yet: the
            # column is read as an exception list, and a page of zeroes hides
    ...
    def get_report_data(self, options):
        ...
        # invoice: a partly-paid invoice only still owes what is left of it.
        ...
        if options.get("include_overdue"):
            ...
            lines.append(
                self._kmitl_period_line(
                    _("Due Date"), options.get("date_from"), options.get("date_to")
                )
            )
    ...
    def get_report_note(self):
        return _(
            "Amount Due is what is left of the invoice, so a partly-paid "
            "invoice is carried at its remaining balance. Days Overdue is "
            "counted from today."
        )
    ...
    def action_print_pdf(self, options):
        return self._kmitl_report_action(
            options,
            CARRIER_MODEL,
            "finance_kmitl_reports.action_report_receivable_due_pdf",
        )
    ...
    def action_export_xlsx(self, options):
        return self._kmitl_report_action(
            options,
            CARRIER_MODEL,
            "finance_kmitl_reports.action_report_receivable_due_xlsx",
        )


class ReceivableDueReportWizard(models.TransientModel):
    _name = CARRIER_MODEL
    _description = "KMITL Receivable Due Report Carrier"
...
class ReceivableDueReportXlsx(models.AbstractModel):
    _name = "report.finance_kmitl_reports.receivable_due_report_xlsx"
    _description = "KMITL Receivable Due Report XLSX"
    ...
    def generate_xlsx_report(self, workbook, data, objs):
        ...
        self.env["finance_kmitl_reports.report.base"]._kmitl_write_xlsx(
            ...
            REPORT_MODEL,
            _("Receivables Due"),
            _("Receivables Due Report"),
            "amount_residual",
        )


class ReceivableDueReportPdf(models.AbstractModel):
    _name = "report.finance_kmitl_reports.receivable_due_report_pdf"
    _description = "KMITL Receivable Due Report PDF"
