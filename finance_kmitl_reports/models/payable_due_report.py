...
class PayableDueReportXlsx(models.AbstractModel):
    ...
    def generate_xlsx_report(self, workbook, data, objs):
        ...
        sheet = workbook.add_worksheet(_("Payables Due"))
        ...
        # The grand total is the outstanding balance, which is not the last
        # column here — Days Overdue is.
        total_col = [key for key, _h, _a, _w in columns].index("amount_residual")

        sheet.merge_range(0, 0, 0, last_col, company.display_name, bold)
        sheet.merge_range(1, 0, 1, last_col, _("Payables Due Report"), bold)
        row_index = 2
