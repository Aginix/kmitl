...
- **D6** `payment_type` removed; borrowing money (`advance_payment`) becomes a post-approval action; the disbursement bridge stops branching on `payment_type`. *(Amended by [ADR-0002](0002-payment-type-per-actual-row.md): `payment_type` returns at the `actual` stage, per allocation row, and routes billing by type — the reasoning here still holds for the plan, not the actual.)*
