...
**Request Attachments (เอกสารแนบ)**:
Two untyped buckets, one per stage, both uploaded through the request's own `res_model`/`res_id` — the plan-stage `attachment_ids` (the Expense Plan page) and the disbursement-stage `disbursement_attachment_ids` (**เอกสารแนบอื่น ๆ**, on the Actual Expenses page). Because the upload route cannot tell them apart, `ir.attachment.is_disbursement_evidence` is the discriminator: the `many2many_binary_disbursement` widget sets it client-side on upload, `write()` re-asserts it server-side, and `attachment_ids` filters it out. Both live here, not in `agx_approval_disbursement` — that bridge only *clones* them onto the DR it creates, and `agx_approval_document_checklist` slots its typed list above the second bucket.
_Avoid_: disbursement evidence as a disbursement-module concept, chatter attachments
