...
**Select vs Reserve (เลือกงบ vs จองงบ)**:
Two distinct capabilities the view used to fuse into one gate. **Select** — filling `budget_account_id` and the four dimension fields — is open to any requester who can edit the request (`is_budget_editable`), as plain dropdowns (no balance exposure). **Reserve** — clicking "Reserve Budget" to mint the `budget.commitment` that actually locks availability — stays behind `budget.group_budget_commitment`, both view-gated (the button, the mode radio, the chart picker, the draw-down field) and guarded server-side in `action_reserve_budget`. A category-pinned budget code still constrains selection (`budget_account_domain`, computed from `_reservation_account_domain`). See [ADR-0007](docs/adr/0007-requester-selects-budget-officer-reserves.md); `purchase_request_budget` mirrors the same split.
_Avoid_: assuming budget.group_budget_commitment gates the whole budget block — since ADR-0007 it only gates reservation, not selection
