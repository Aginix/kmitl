...
class BudgetAppropriationF9WExpense(models.AbstractModel):
    """
    Budget Appropriation F9-W Expense Report - Pivot table by departments and activity plans.
    ...
    _name = "budget.appropriation.f9w.expense"
    _description = "Budget Appropriation F9-W Expense Report (Pivot)"
    ...
    def get_data(self, report_id):
        ...
        dept_column_totals = self._build_dept_column_totals(report, dept_map, activity_path_map, reserve_fund_account_ids)

        # Build comparison totals if compare_report_id exists
        compare_report = report.compare_report_id
        compare_totals = {}
        if compare_report:
            compare_totals = self._build_dept_column_totals(compare_report, dept_map, activity_path_map, reserve_fund_account_ids)
    ...
    def _build_dept_column_totals(self, report, dept_map, activity_path_map, reserve_fund_account_ids):
        """
        Build (dept_id, column_code) -> balance mapping for a report.
        ...
        """
        lines = report.expense_appropriation_ids.mapped("line_ids")
        totals = {}
