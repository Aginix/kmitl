...
class BudgetAppropriationF8WExpense(models.AbstractModel):
    """
    Budget Appropriation F8-W Expense Report - Aggregate expenses by departments then activity plans.
    ...
    Business Purpose:
        ...
        Includes comparison with compare_report_id.
    ...
    _name = "budget.appropriation.f8w.expense"
    _description = "Budget Appropriation F8-W Expense Report"
    ...
    def get_data(self, report_id):
        ...
        dept_activity_totals = self._build_dept_activity_totals(report, dept_map, activity_path_map)

        # Build comparison totals if compare_report_id exists
        compare_report = report.compare_report_id
        compare_totals = {}
        if compare_report:
            compare_totals = self._build_dept_activity_totals(compare_report, dept_map, activity_path_map)
    ...
    def _build_dept_activity_totals(self, report, dept_map, activity_path_map):
        """
        Build (dept_id, xml_id) -> balance mapping for a report.
