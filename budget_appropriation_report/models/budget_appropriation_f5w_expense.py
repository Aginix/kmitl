...
class BudgetAppropriationF5WExpense(models.AbstractModel):
    """
    Budget Appropriation F5-W Expense Report - Aggregate expenses by activity plans.

    Business Purpose:
        Generates F5-W expense report data by aggregating budget appropriation
        lines by activity plans (แผนงาน). Includes comparison with compare_report_id.
    ...
    _name = "budget.appropriation.f5w.expense"
    _description = "Budget Appropriation F5-W Expense Report"
    ...
    def get_data(self, report_id):
        ...
        activity_totals = self._build_activity_totals(report)

        # Build comparison totals if compare_report_id exists
        compare_report = report.compare_report_id
        compare_totals = {}
        if compare_report:
            compare_totals = self._build_activity_totals(compare_report)
    ...
    def _build_activity_totals(self, report):
        """
        Build xml_id -> balance mapping for a report.
