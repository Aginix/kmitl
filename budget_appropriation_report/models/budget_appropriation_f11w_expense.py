...
class BudgetAppropriationF11WExpense(models.AbstractModel):
    """
    Budget Appropriation F11-W Expense Report - Pivot table by departments and funds.
    ...
    _name = "budget.appropriation.f11w.expense"
    _description = "Budget Appropriation F11-W Expense Report (Pivot by Funds)"
    ...
    def get_data(self, report_id):
        ...
        dept_column_totals = self._build_dept_column_totals(report, dept_map, fund_path_map)

        # Build comparison totals if compare_report_id exists
        compare_report = report.compare_report_id
        compare_totals = {}
        if compare_report:
            compare_totals = self._build_dept_column_totals(compare_report, dept_map, fund_path_map)
    ...
    def _build_dept_column_totals(self, report, dept_map, fund_path_map):
        """
