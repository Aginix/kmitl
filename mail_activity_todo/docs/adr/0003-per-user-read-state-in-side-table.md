...
- New model `todo.read` (in the `mail_activity_todo` core) — `activity_id` (`mail.activity`, `ondelete=cascade`), `user_id` (`res.users`), `read_date`; unique on (`activity_id`, `user_id`).
