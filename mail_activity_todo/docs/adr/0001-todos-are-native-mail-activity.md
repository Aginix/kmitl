...
> Packaging note: the engine described here later shipped split across
> `mail_activity_todo` (core) + `mail_activity_todo_role_unit` (group routing) +
> per-app bridges, with models `todo.read` / `todo.log`. See
> [ADR-0005](./0005-split-into-reusable-core-layer-and-bridges.md). The
> mechanism below is unchanged.
