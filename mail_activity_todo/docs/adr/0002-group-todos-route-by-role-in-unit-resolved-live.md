...
## Update (2026-08-12): view scope vs notification scope split ([ADR-0007](./0007-split-notification-scope-from-view-scope.md))

The `role ∩ OU` recipient rule above still governs **who can see** a group Todo (view scope). What was originally one axis has been split — the *primary* inbox and the systray badge now use a per-user **notification scope** (a subset of the view-scope OU list), so a wide-access user (e.g. `group_manager_operating_unit`) is not flooded by every OU's work just because they can look at every OU. Group Todos outside the notification scope stay accessible under an "Oversight" sibling menu. Empty notification scope = fall back to view scope, so untouched users are unaffected.
