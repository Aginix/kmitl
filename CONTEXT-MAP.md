...
- **Work Acceptance Portal Review → Todos**: `request_validation()` schedules an Approval Todo per committee user; `activity_feedback` closes it when that committee member sets their `status`; portal WA + PO URLs ride on the Todo as Action Links resolved live per recipient (`purchase_work_acceptance_portal` ADR-0001, ADR-0002).
