...
- **Work Acceptance Portal Review → Todos**: `request_validation()` schedules an Approval Todo per committee user; `activity_feedback` closes it when that committee member sets their `status`; portal WA + PO URLs live as smart buttons on the WA backend form, reachable via the Todo's primary "Open Source" action (`purchase_work_acceptance_portal` ADR-0001, ADR-0003).
