...
To let users review Todos they have finished — which the inbox cannot show, because core Odoo **deletes** a `mail.activity` when it is marked done — we snapshot every completed Todo into a dedicated `kmitl.todo.log` table. `mail.activity._action_done` is overridden to record the activity (only those carrying a `todo_category`) just before core unlinks it. The log captures the routing (assignee or responsible role + operating unit), the source record, the category, the deadline, and *who* completed it *when*.
...
- New model `kmitl.todo.log` (read-only to users; written via `sudo()` from the `_action_done` hook).
