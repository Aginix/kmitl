# kmitl_todo — Roadmap & Backlog

Forward-looking backlog for the unified Todos inbox. Design rationale for what
already exists lives in [`docs/adr/`](./adr) (ADR-0001..0004) and the glossary
...
- Core: Todos = native `mail.activity`; role-in-unit group routing resolved live
  (ADR-0001/0002); per-user read state in `kmitl.todo.read` (ADR-0003);
  completed-Todo history in `kmitl.todo.log` (ADR-0004).
...
- Review fixes: `kmitl.todo.log` record rule, server-side category gate, GC
