...
from odoo.addons.finance_kmitl.models.kmitl_payment_subject import (
    _paying_account_domain,
)
...
class DisbursementPaymentLine(models.Model):
    ...
    payee-level row that becomes exactly one ``account.payment`` against exactly
    one posted bill — which is why the paying account lives here rather than on
    the request line, where the same decision would have to be copied onto every
    item of a payee and then validated back into agreement. Here the invariant
    is structural.

    The amounts are read from the **posted bill** — the money that will actually
    leave, net of withholding tax — not from the sum of the request lines, which
    is only what was asked for.

    Thai keeps this apart from the payment it becomes by document weight: this is
    a **รายการ**จ่ายเงิน (a row), the ``account.payment`` is an **ใบ**จ่ายเงิน
    (a document).
    """
    ...
    partner_bank_id = fields.Many2one(
        ...
        help="The payee's account the money is transferred into. Defaulted from "
        "the bill; it is what the payment and the bank export carry.",
    )
    ...
    allowed_paying_account_ids = fields.Many2many(
        ...
        help="What the paying account may be set to, so the subject's policy is "
        "enforced while choosing instead of rejected afterwards.",
    )
    paying_account_match = fields.Selection(
        selection=[
            ("bank", "Payee's Own Bank"),
    ...
    def _compute_allowed_paying_account_ids(self):
        """A subject that lists allowed accounts narrows the choice to them; a
        subject that lists none allows every paying account.

        The list spans methods on purpose: moving a payee from เงินโอน to เช็ค
        *is* picking a different paying account.
        """
        every = self.env["account.payment.method.line"].search(
            _paying_account_domain(self)
        )
        for line in self:
            allowed = line.request_id.payment_subject_id.allowed_paying_account_ids
    ...
    def _onchange_paying_account_id(self):
        ...
        Only fires from the UI: the subject-derived assignment writes through the
        ORM, which does not run onchange, so it never mislabels itself.
        """
        for line in self:
            line.paying_account_match = "manual" if line.paying_account_id else False

    @api.onchange("partner_bank_id")
    def _onchange_partner_bank_id(self):
        """Re-derive the paying account when the payee's bank changes.

        The derivation has two inputs — the subject and the payee's own bank — so
        a new bank makes an auto-matched paying account, and the badge that says
        it matches, a lie. Rows a person picked by hand are left alone: they
    ...
    def _refresh_amounts(self):
        ...
        """
        for line in self.filtered(lambda row: not row.payment_id):
    ...
    def write(self, vals):
        if any(field in vals for field in BANKING_FIELDS):
            for line in self:
                if line._banking_editable():
                    continue
                raise UserError(
                    _(
                        "The payment of %(payee)s can no longer be changed: %(reason)s",
                        ...
                        reason=(
                            ...
                            if line.payment_id
                                ...
                                "the request is not open for audit or payment review."
                            )
                        ),
                    )
                )
