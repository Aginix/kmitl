...
- Because approval is not restricted to `approver_id`, a manager other than the assignee can approve. The To-Do is still closed (the helpers `sudo()`, see ADR-0015), but it will read as done by someone who was never the assignee. That is accepted for now; narrowing `action_approve` is the follow-up if it matters.
