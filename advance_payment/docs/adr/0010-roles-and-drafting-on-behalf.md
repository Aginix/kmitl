...
- `group_advance_payment_loan_officer` (**new**): the standalone oversight tier — owns every workflow action (verify, reset-to-draft, accept-report, reconcile/return-lines, due-date, bank correction). Implies `user` (sees all) and `responsible` (return-line powers), so the single oversight person gets both without touching `responsible` directly.
- `group_advance_payment_responsible` (unchanged): return-line approve/reject buttons only.

A new per-record field, `loan_officer_id` (Many2one `res.users`, domain = loan-officer group members, defaulted to the sole member when exactly one exists, manager-editable) — "เจ้าหน้าที่งานเงินยืม" — records who is responsible for a given agreement.
...
- `is_officer` is renamed `is_loan_officer` throughout (model + views); a new `is_manager` computed field is added to gate `loan_officer_id`'s edit rights in the form.
