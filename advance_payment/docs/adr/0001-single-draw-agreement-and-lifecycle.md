...
- **Reset to draft (ตั้งกลับเป็นแบบร่าง).** The borrower may pull a not-yet-approved request (`to_verify` / `to_approve`) back to `draft`, keeping the ADV number (`action_recall`). Two more paths reach `draft` under the same UI verb — the loan officer's ส่งกลับแก้ไข from `to_verify` and the manager's un-cancel from `cancel`; see the CONTEXT.md entry for the split. The domain term is *reset to draft*, not *ดึงกลับ*, which is reserved for e-Saraban (ADR-0011).
