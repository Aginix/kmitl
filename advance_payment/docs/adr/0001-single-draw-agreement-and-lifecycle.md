...
The lifecycle is `draft → to_verify → to_approve → waiting_transfer → in_progress → to_verify_report → to_reconcile → done`, with negative states `rejected` and `cancel`. (The settle stage — `to_verify_report` / `to_reconcile` — is detailed in ADR-0003.)
...
- Keeps the money model simple (one agreement = one debt) and reuses existing rails: `advance_payment_tier_validation` for approval and the `finance_kmitl` bank-export gate for disbursement.
...
- **Two review stages.** `to_verify` = finance officer checks the paper documents (Verify); `to_approve` = tier-validation Endorse → Approve chain, routed by org (Faculty vs สนอ.).
- **Edit rights before approval.** Material fields (amount, loan type, bank, borrower, department, analytic dimensions, source reference) lock at submit. The creator may still edit `loan_reason` + attachments while in `to_verify`. A `group_advance_payment_officer` may reset-to-draft and edit freely while in `to_verify`.
- **Recall (ดึงกลับ).** The creator may pull a not-yet-approved request (`to_verify` / `to_approve`) back to `draft`, keeping the ADV number.
