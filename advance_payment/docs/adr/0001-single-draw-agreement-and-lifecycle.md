...
- **Edit rights before approval.** Material fields (amount, loan type, bank, borrower, department, analytic dimensions, source reference) lock at submit. The creator may still edit `loan_reason` + attachments while in `to_verify`. A `group_advance_payment_user` (finance officer in domain language) may reset-to-draft and edit freely while in `to_verify`. Security model: own → viewer → user → manager.
