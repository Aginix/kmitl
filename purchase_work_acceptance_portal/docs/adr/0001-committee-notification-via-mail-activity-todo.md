...
- **Portal URLs still work because of smart buttons on the WA form.** The committee member's `committee_token` (and the WA's `wa_token` on PO links) is per-recipient and per-document; storing pre-rendered URLs in a side table was the inbox's reason to exist. Smart buttons resolve URLs live against `env.user` on click, so the same need is met without a side table — see [ADR-0003](./0003-portal-links-as-smart-buttons.md).
