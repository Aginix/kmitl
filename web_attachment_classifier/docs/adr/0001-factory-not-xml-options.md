...
`ir.attachment` with scope + ordering driven by a Mapping table
(`ir.attachment.document.type.rel`), and the `many2many_binary`
standard widget itself is patched via `patch()` so no widget name
change is needed anywhere. See ADR-0002 for the current architecture
and why.
