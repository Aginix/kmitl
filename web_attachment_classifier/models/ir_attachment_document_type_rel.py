...
class IrAttachmentDocumentTypeRel(models.Model):
    """Line inside an `attachment.classifier.model.config` — one doctype
    on the config's model, at a given `sequence`.

    The classifier widget resolves the doctype list for the current form
    ...
    config_id = fields.Many2one(
        comodel_name="attachment.classifier.model.config",
        string="Model Config",
        required=True,
        ondelete="cascade",
    )
